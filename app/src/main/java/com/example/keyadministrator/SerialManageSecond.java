...
public class SerialManageSecond {
    private static SerialManageSecond instance;
    ...
    private SerialManageSecond() {
        scheduledExecutor = Executors.newScheduledThreadPool(8);//初始化8个线程
    }

    public static SerialManageSecond getInstance() {
        if (instance == null) {
            synchronized (SerialManageSecond.class) {
                if (instance == null) {
                    instance = new SerialManageSecond();
                }
            }
        }
        return instance;
